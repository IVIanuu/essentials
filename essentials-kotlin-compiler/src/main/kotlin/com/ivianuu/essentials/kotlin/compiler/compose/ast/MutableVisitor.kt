...
open class MutableVisitor {
    ...
    private fun <T : Node?> Node?.visitChildren(v: List<T>, ch: ChangedRef): List<T> =
        ch.sub { newCh ->
            val newList =
                v.map { orig -> visit(orig, this!!, newCh).also { newCh.markIf(it, orig) } }
    ...
    companion object {
        fun <T : Node> preVisit(v: T, fn: (v: Node?, parent: Node) -> Node?) =
            object : MutableVisitor() {
                override fun <T : Node?> preVisit(v: T, parent: Node): T = fn(v, parent) as T
            }.visit(v, v)

        fun <T : Node> postVisit(v: T, fn: (v: Node?, parent: Node) -> Node?) =
            object : MutableVisitor() {
