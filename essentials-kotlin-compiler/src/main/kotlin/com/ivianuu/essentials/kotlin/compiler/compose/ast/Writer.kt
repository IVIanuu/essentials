...
open class Writer(val app: Appendable = StringBuilder()) : Visitor() {
    ...
    protected fun appendName(name: String): Writer {
        return if (name.shouldEscapeIdent) append("`$name`") else append(name)
    }
        ...
        v?.apply {
            ...
            if (replacedSource != null) append(replacedSource)
                ...
                is Node.Import -> {
                    ...
                    if (wildcard) append(".*") else if (alias != null) append(" as ").appendName(
                        alias!!
                    )
                }
                is Node.Command -> {
                ...
                is Node.Decl.Structured -> childMods().also {
                    ...
                    if (form != Node.Decl.Structured.Form.COMPANION_OBJECT || name != "Companion") appendName(
                        name
                    )
                    ...
                    if (members.isNotEmpty()) lineEnd(" {").indented {
                        ...
                        val enumEntries =
                            members.map { it as? Node.Decl.EnumEntry }.takeWhile { it != null }
                ...
                is Node.Decl.Property.Var -> {
                    if (name != "_") appendName(name) else append("_")
                    if (type != null) append(": ").also { children(type) }
                }
                ...
                is Node.Expr.While -> {
                    if (!doWhile) append("while (").also { children(expr) }.append(") ").also {
                        children(
                            body
                        )
                    }
                    else append("do ").also { children(body) }.append(" while (").also {
                        children(
                            expr
                        )
                    }.append(')')
                ...
                is Node.Expr.BinaryOp -> {
                    ...
                    val noSep =
                        oper is Node.Expr.BinaryOp.Oper.Token && (oper as Node.Expr.BinaryOp.Oper.Token).token.let {
    ...
    protected fun Node.WithModifiers.childModsBeforeType(ref: Node.TypeRef) = this@Writer.also {
        if (mods.isNotEmpty()) {
            ...
            val lastAnn = (mods.lastOrNull() as? Node.Modifier.AnnotationSet)?.anns?.singleOrNull()
    ...
    protected inline fun Node.children(vararg v: Node?) =
        this@Writer.also { v.forEach { visitChildren(it) } }
    ...
    protected fun Node.childrenLines(
        v: Node?,
        extraEndLines: Int = 0
    ) =
        this@Writer.also { if (v != null) childrenLines(listOf(v)) }
    ...
    protected fun Node.childrenLines(
        ...
        this@Writer.also {
            v.forEachIndexed { index, node ->
                ...
                if (stmtRequiresEmptyBraceSetBeforeLineEnd(
                        node,
                        v.getOrNull(index + 1)
                    )
                ) append(" {}")
    ...
    protected fun stmtRequiresEmptyBraceSetBeforeLineEnd(v: Node?, next: Node?): Boolean {
        ...
        if (v !is Node.Stmt.Decl || v.decl !is Node.Decl.Structured || (v.decl as Node.Decl.Structured).members.isNotEmpty() ||
            (v.decl as Node.Decl.Structured).form != Node.Decl.Structured.Form.CLASS
        ) return false
        if (next !is Node.Stmt.Expr || (next.expr !is Node.Expr.Paren &&
                    (next.expr !is Node.Expr.Annotated || (next.expr as Node.Expr.Annotated).expr !is Node.Expr.Paren))
        ) return false
        return true
    }
    ...
    protected fun stmtHasTrailingLambdaAmbiguity(v: Node?, next: Node?): Boolean {
        ...
        if (v !is Node.Stmt.Expr || v.expr !is Node.Expr.Call || (v.expr as Node.Expr.Call).lambda != null) return false
    ...
    companion object {
        ...
        fun write(v: Node): String {
            val builder = StringBuilder()
            Writer(builder).write(v)
