package com.ivianuu.essentials.kotlin.compiler.compose.ast.psi

import com.ivianuu.essentials.kotlin.compiler.compose.ast.COMMAND_PREFIX
import com.ivianuu.essentials.kotlin.compiler.compose.ast.ExtrasMap
...
open class Converter {
    ...
    open fun convertAnnotationSets(v: KtElement): List<Node.Modifier.AnnotationSet> =
    ...
    open fun convertAnnotationSetTarget(v: KtAnnotationUseSiteTarget) =
    ...
    open fun convertDoubleColonRefCallable(v: KtCallableReferenceExpression) =
    ...
    open fun convertDoubleColonRefClass(v: KtClassLiteralExpression) =
    ...
    open fun convertModifiers(v: KtModifierList?) =
    ...
    open fun convertPrimaryConstructor(v: KtPrimaryConstructor) =
    ...
    open fun convertStringTmplElem(v: KtStringTemplateEntry) = when (v) {
        ...
        is KtSimpleNameStringTemplateEntry ->
            Node.Expr.StringTmpl.Elem.ShortTmpl(
                v.expression?.text ?: error("No short tmpl text")
            ).map(v)
    ...
    open fun convertValueArgs(v: KtValueArgumentList?) =
    ...
    open class WithExtras : Converter(), ExtrasMap {
        ...
        override fun extrasBefore(v: Node) =
            nodesToPsiIdentities[v]?.let { extrasBefore[it] } ?: emptyList()

        override fun extrasWithin(v: Node) =
            nodesToPsiIdentities[v]?.let { extrasWithin[it] } ?: emptyList()

        override fun extrasAfter(v: Node) =
            nodesToPsiIdentities[v]?.let { extrasAfter[it] } ?: emptyList()
        ...
        override fun onNode(node: Node, elem: PsiElement) {
            ...
            convertExtras(beforeElems).map {
                ...
                if (it is Node.Extra.Comment && !it.startsLine && it.text.startsWith("//")) it.copy(
                    startsLine = true
                )
                else it
        ...
        open fun nodeExtraElems(elem: PsiElement): Triple<List<PsiElement>, List<PsiElement>, List<PsiElement>> {
            ...
            if (seenNonWs && indexOfFirstNewline != -1) after =
                after.subList(0, indexOfFirstNewline)

            return Triple(before, within, after)
        }

        open fun convertExtras(elems: List<PsiElement>): List<Node.Extra> =
            elems.mapNotNull { elem ->
                ...
                if (!seenExtraPsiIdentities.add(elemId)) null else when (elem) {
                    ...
                    is PsiComment -> Node.Extra.Comment(
                        text = elem.text,
                        startsLine = ((elem.prevSibling
                            ?: elem.prevLeaf()) as? PsiWhiteSpace)?.textContains('\n') == true,
                        endsLine = elem.tokenType == KtTokens.EOL_COMMENT ||
                                ((elem.nextSibling
                                    ?: elem.nextLeaf()) as? PsiWhiteSpace)?.textContains('\n') == true
                    ).map(elem)
                    else -> null
                }
            }
    ...
    companion object : Converter() {
        ...
        internal val KtTypeReference.names
        ...
        internal val KtExpression?.block
...
internal val PsiElement.ast: Node
    get() = when (this) {
