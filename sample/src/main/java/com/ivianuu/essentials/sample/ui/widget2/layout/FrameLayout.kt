...
class FrameLayoutWidget(
    children: List<Widget>,
    key: Any? = null
) : ViewGroupWidget<FrameLayout>(children, key) {

    constructor(
        key: Any? = null,
        children: FrameLayoutBuilder.() -> Unit
    ) : this(FrameLayoutBuilder().apply(children).children, key)
...
class FrameLayoutParams(
    ...
    override fun applyViewProps(context: BuildContext, view: View) {
        val lp = view.layoutParams.cast<FrameLayout.LayoutParams>()
