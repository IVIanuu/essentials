...
class SecureSettingsInstructionsController : PrefsController() {
    ...
    private val key by bindKey<SecureSettingsInstructionsKey>()

    override fun epoxyController() = epoxyController {
        preference {
            key("secure_settings_header")
            summaryRes(R.string.pref_summary_secure_settings_header)
        }

        preference {
            key("secure_settings_step_1")
            titleRes(R.string.pref_title_secure_settings_step_1)
            summaryRes(R.string.pref_summary_secure_settings_step_1)
            clickable(false)
        }

        preference {
            key("secure_settings_step_two")
            titleRes(R.string.pref_title_secure_settings_step_2)
            summaryRes(R.string.pref_summary_secure_settings_step_2)
            clickable(false)
        }

        preference {
            key("secure_settings_step_3")
            titleRes(R.string.pref_title_secure_settings_step_3)
        }

        preference {
            key("secure_settings_link_gadget_hacks")
            icon(R.drawable.ic_link)
            summaryRes(R.string.pref_summary_secure_settings_link_gadget_hacks)
            urlClickListener { "https://youtu.be/CDuxcrrWLnY" }
        }

        preference {
            key("secure_settings_link_lifehacker")
            icon(R.drawable.ic_link)
            summaryRes(R.string.pref_summary_secure_settings_link_lifehacker)
            urlClickListener { "https://lifehacker.com/the-easiest-way-to-install-androids-adb-and-fastboot-to-1586992378" }
        }

        preference {
            key("secure_settings_link_xda")
            icon(R.drawable.ic_link)
            summaryRes(R.string.pref_summary_secure_settings_link_xda)
            urlClickListener { "https://www.xda-developers.com/install-adb-windows-macos-linux/" }
        }

        preference {
            key("secure_settings_step_4")
            titleRes(R.string.pref_title_secure_settings_step_4)
